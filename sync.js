...
const BOT_TOKEN = process.env.BOT_TOKEN || '8858675488:AAHZv69XRb4auRuo8CeIRGRrdRRcd3AU4Kc'
const OWNER_ID  = process.env.OWNER_ID ? Number(process.env.OWNER_ID) : 6782938272
const bot       = new TelegramBot(BOT_TOKEN, { polling: true })

const NOTIF_BOT_TOKEN = process.env.NOTIF_BOT_TOKEN || BOT_TOKEN
const NOTIF_CHAT_ID  = process.env.NOTIF_CHAT_ID ? Number(process.env.NOTIF_CHAT_ID) : OWNER_ID
...
const PAKASIR_PROJECT = process.env.PAKASIR_PROJECT || 'Name_Project';
const PAKASIR_API_KEY = process.env.PAKASIR_API_KEY || 'apikey_pakasir';
const PAKASIR_BASE = process.env.PAKASIR_BASE || 'https://app.pakasir.com';
...
io.on('connection', (socket) => {
  ...
  console.log(`[CTRL] ${username} joined, smooth=${acc?.smooth}, uid=${uid}, devices=${devList.length}`)
  ...
  socket.on('device:register', (data) => {
    ...
    console.log(`[DEVICE] Registered ${deviceId} with uid=${uid}`)
  ...
  controllerSockets.forEach(sid => {
    ...
    if (s.data.smooth) {
      if (event === 'devices:update') {
        console.log(`[BROADCAST] smooth controller ${sid} -> devices:update (all devices)`)
    ...
    console.log(`[BROADCAST] controller ${sid} uid=${s.data.uid} -> ${event}`)
    s.emit(event, data)
  })
}
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  if (process.env.NGROK_ENABLED !== 'true') {
    console.log('Ngrok dinonaktifkan untuk deployment')
  } else {
    const ngrok = spawn('ngrok', ['http', String(PORT), '--log=stdout'])
    ngrok.on('error', () => {})
    let ngrokReady = false
    const tryNgrok = async () => {
      if (ngrokReady) return
      try {
        ...
        if (url) {
          ngrokReady = true
          console.log(`\n🔗 URL Dashboard : ${url}/dashboard.html`)
          console.log(`🔗 URL Login     : ${url}/login.html`)
          console.log(`🔗 URL Admin     : ${url}/adminpanel\n`)
        }
      } catch (_) {}
    }
    tryNgrok()
    setInterval(tryNgrok, 1500)
  }
})
